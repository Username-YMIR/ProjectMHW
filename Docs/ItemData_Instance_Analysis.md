...
2. 공격 시작 시점의 버프, 장착 효과 등이 반영된 소스 어트리뷰트 값을 DamageSpec에 담아 두고, 피격 시점에도 같은 값을 적용하려는 목적
3. 추후 투사체 무기처럼 공격 생성 시점과 실제 피격 시점이 분리되는 구조에도 같은 DamageSpec 전달 모델을 확장 적용하려는 목적
4. 무기 액터는 충돌/비주얼/공격 상태를 담당하고, 플레이어는 장착 상태 머신과 ASC를 담당하려는 목적
